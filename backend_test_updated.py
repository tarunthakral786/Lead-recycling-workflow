...
class LeadTrackAPITester:
        ...
        try:
            ...
            if success:
                ...
                if hasattr(self, 'original_pp') and hasattr(self, 'original_mc_smf'):
                    ...
                    requests.put(f"{self.api_url}/admin/recovery-settings", json=restore_settings, headers=headers, timeout=10)
    ...
    def create_test_image(self):
        ...
        png_data = b'\x89PNG\r\n\x1a\n\rIHDR\x01\x01\x08\x02\x90wS\xde\tpHYs\x0b\x13\x0b\x13\x01\x9a\x9c\x18\x0cIDATx\x9cc```\x04\x01\xdd\x8d\xb4\x1cIEND\xaeB`\x82'
        return io.BytesIO(png_data)

    def test_create_refining_entry(self):
        """Test creating a new refining entry with images"""
        if not self.token:
            self.log_test("Create Refining Entry", False, "No authentication token")
            return False
            
        headers = {'Authorization': f'Bearer {self.token}'}
        
        # Prepare batch data for new refining API
        batches_data = [{
        ...
        # Prepare files (5 images per batch)
        files = [
            ('files', ('test_lead.png', self.create_test_image(), 'image/png')),
            ('files', ('test_dross1.png', self.create_test_image(), 'image/png')),
            ('files', ('test_dross2.png', self.create_test_image(), 'image/png')),
            ('files', ('test_dross3.png', self.create_test_image(), 'image/png')),
            ('files', ('test_pure.png', self.create_test_image(), 'image/png'))
        ]
        
        try:
            response = requests.post(
                f"{self.api_url}/refining/entries", 
                data=form_data, 
                files=files, 
                headers=headers, 
                timeout=30
            ...
            self.log_test("Create Refining Entry", success, details)
            return success
        except Exception as e:
            self.log_test("Create Refining Entry", False, f"Error: {str(e)}")
            return False
