...
import { Camera, ArrowLeft, Plus, X, Calendar, Package, Check } from 'lucide-react';
import { compressImage } from '@/utils/imageCompression';

const BACKEND_URL = process.env.REACT_APP_BACKEND_URL;
const API = `${BACKEND_URL}/api`;

export default function RMLReceivedSantoshPage({ user }) {
  ...
  const handleSubmit = async () => {
    ...
    if (incompleteBatches.length > 0) {
      toast.error('Please complete all fields (quantity, pieces, SB%, and at least one photo)');
      return;
    }
    ...
    try {
      ...
      await axios.post(`${API}/rml-received-santosh`, form, {
        headers: {
  ...
  const getSKUPreview = (batch) => {
    ...
    const remarks = batch.remarks || 'SANTOSH';
    const sb = parseFloat(batch.sb_percentage);
    return `SANTOSH-${remarks}, ${sb}%, ${entryDate}`;
  };

  return (
    <div className="min-h-screen bg-slate-100">
      {/* Header */}
      <div className="bg-green-600 border-b border-green-700 shadow-sm">
        <div className="max-w-3xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <Button
              onClick={() => navigate('/')}
              data-testid="back-button"
              className="h-12 px-4 bg-green-700 hover:bg-green-800 text-white border-0 rounded-lg"
            >
              <ArrowLeft className="w-5 h-5" />
            </Button>
            <div>
              <h1 className="text-2xl font-bold text-white">RML Received SANTOSH</h1>
              <p className="text-base text-green-100">{batches.length} batch(es)</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <Card className="bg-green-50 border-green-200 rounded-xl p-4 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            />
            <p className="text-sm text-green-600">Select date for this entry</p>
          </div>
        </Card>
        ...
        <Card className="bg-blue-50 border-blue-200 rounded-xl p-4 mb-6">
          <div className="flex gap-3">
            ...
            <div>
              <p className="text-base font-semibold text-blue-900">RML Received from SANTOSH</p>
              <p className="text-sm text-blue-700">Enter received RML from recycling receivable. This will deduct from your "Lead Receivable" and create SKUs for use in Refining.</p>
            </div>
          </div>
        </Card>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    className="h-14 text-xl px-4 w-full border-2 border-slate-200 rounded-lg focus:ring-2 focus:ring-green-200 focus:border-green-400"
                    placeholder="0.00"
                  />
                </div>
              ...
              <div>
                ...
                <Textarea
                  ...
                  className="min-h-[80px] text-lg px-4 py-3 w-full border-2 border-slate-200 rounded-lg focus:ring-2 focus:ring-green-200 focus:border-green-400"
